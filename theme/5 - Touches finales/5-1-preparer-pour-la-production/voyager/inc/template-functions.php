...
function voyager_nav_menu_item_classes( $classes, $menu_item, $args, $depth ){
    switch ( $args->theme_location ) {
        ...
        case 'menu-2':
            ...
            break;
        case 'menu-3':
            $classes[] = 'mb-0 flex align-center';
            break;
    }
    return $classes;
}
...
function voyager_menu_item_title( $title, $menu_item, $args, $depth ){
	...
	if ( ! empty( $icon_keys ) ){
        ...
        $title = sprintf( '<span class="mr-1">%s</span><span class="menu-item-title">%s</span>', $icon, $title );
	}
    return $title;
}
...
function voyager_submenu_class( $classes, $args, $depth ){
    switch ( $args->theme_location ) {
        ...
        case 'menu-2':
            ...
            break;
        case 'menu-3':
            $classes = array_merge( $classes, array( 'footer-submenu', 'naked', 'flex', 'align-center', 'flex-wrap' ) );
            break;
    }
    return $classes;
}
...
/**
 * Styles the header image and header text.
 */
function voyager_header_style() {
    $header_text_color = get_header_textcolor();
    $hero_text_color   = get_theme_mod( 'voyager_hero_text_color', '#e0e0e0' );
    ...
    ?>
        <style type="text/css" id="voyager_header_styles">
            ...
            .title-block {
                color: <?php echo sanitize_hex_color( $hero_text_color ); ?>
            }
            .page-title {
                color: <?php echo sanitize_hex_color( $hero_title_color ); ?>
            }
        </style>
    <?php
}
...
add_filter( 'comment_form_fields', 'voyager_comments_form_fields', 10, 1 );
/**
 * Filters the comments form fields
 * Reorders them, putting `comment` and `cookies` at the bottom
 * 
 * @param   array  $fields
 * @return  array  $fields
 */
function voyager_comments_form_fields( $fields ){
...
add_filter( 'get_the_archive_title_prefix', 'voyager_archive_title_prefix', 10, 1 );
/**
 ...
 */
function voyager_archive_title_prefix ( $prefix ){
    $prefix = '';
    return $prefix;
}